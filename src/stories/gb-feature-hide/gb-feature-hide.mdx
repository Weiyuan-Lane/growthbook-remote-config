...
This component is used to identify if it should be hidden, given some feature flag in [GrowthBook](https://www.growthbook.io/)
...
Under any of the remote config above, as long as a `true` value is returned, the component will be hidden, allowing us to use this in versatile ways
...
```
If `some-key` feature in GrowthBook is configured to `true`, the above will be hidden. Else, it will be shown
